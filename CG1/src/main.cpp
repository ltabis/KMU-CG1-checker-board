...
int main(void)
{
	...
	checkerShader.load("./res/shaders/checker.shader");
	checkerShader.attach("triangle");
	checkerShader.attach("color");
	checkerShader.createExecutable();
	...
	while (!renderer.windowShouldClose()) {
		...
		// Rendering the floor.
		for (auto& plane : floor.board()) {
			checkerShader.setUniform("u_mvp", controller.view() * plane.transform.model());
			renderer.draw(plane, checkerShader);
		}
